...
class LTXVideoAttnProcessor:
    ...
    def __call__(
        ...
        attention_mask_flatten = None
        if attention_mask is not None:
            if cross_view_attn:
                batch_size = batch_size // n_view
                attention_mask = attention_mask.reshape(batch_size, n_view)
                attention_mask_new = attention_mask[:, None, :] * attention_mask[:, :, None]
                attention_mask_new.diagonal(dim1=-2, dim2=-1).fill_(True)
                
                attention_mask_new = attention_mask_new[:, :, None, :, None].repeat(1, 1, sequence_length, 1, sequence_length).reshape(batch_size, n_view * sequence_length, n_view * sequence_length)
                attention_mask = (1 - attention_mask_new) * -1e6
                sequence_length = attention_mask.shape[-1]
            elif image_rotary_emb is not None:
                attention_mask = attention_mask[:, None, None].repeat(1, sequence_length, sequence_length)
                sequence_length = attention_mask.shape[-1]
                
        if attention_mask is not None:
            attention_mask = attn.prepare_attention_mask(attention_mask, sequence_length, batch_size)
            attention_mask = attention_mask.view(batch_size, attn.heads, *attention_mask.shape[-2:])
...
class LTXVideoRotaryPosEmbed(nn.Module):
    ...
    def _prepare_video_coords(
        ...
        if rope_interpolation_scale is not None:
            if isinstance(rope_interpolation_scale[0], torch.Tensor):
                grid[:, 0:1] = grid[:, 0:1] * rope_interpolation_scale[0][:, None, None, None, None] * self.patch_size_t / self.base_num_frames
            else:
...
class LTXVideoTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        if cross_view_attn:
            ...
            nn.init.zeros_(self.attn_view.to_out[0].weight)
            if self.attn_view.to_out[0].bias is not None:
                nn.init.zeros_(self.attn_view.to_out[0].bias)
    ...
    def forward(
        ...
        view_embed: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        batch_size = hidden_states.size(0)
        ...
        if cross_view_attn:
            if view_embed is not None:
                hidden_states = hidden_states + view_embed
            attn_hidden_states = self.attn_view(
        ...
        # hidden_states = hidden_states * self_attention_mask[:, None, None]
        return hidden_states
...
class LTXVideoTransformer3DModel(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.use_view_embed:
            self.view_embed = nn.Parameter(torch.zeros(max_view, inner_dim))
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.use_view_embed and not disable_view_embed:
            view_patterns_dict = {
                1: torch.tensor([
                    [0],
                ], device=temb.device, dtype=torch.long),
                2: torch.tensor([
                    [0, 0],
                    [0, 1],
                ], device=temb.device, dtype=torch.long),
            ...
            view_pattern_id = (rearrange(video_attention_mask, '(b v) -> b v', v=n_view).sum(dim=1)-1).long()
            view_pattern = view_patterns_dict[n_view][view_pattern_id]
            embedded_view = rearrange(self.view_embed[view_pattern], 'b v c -> (b v) c').unsqueeze(1)
        else:
            embedded_view = None
        ...
        for block_idx, block in enumerate(self.transformer_blocks):
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                ...
                hidden_states = block(
                    ...
                    view_embed=embedded_view,
                )
            video_states_buffer.append(hidden_states)
        ...
        # output = output * video_attention_mask[:, None, None]
...
def get_diffusion_model(config, load_weights=True, skip_load_from_pretrain=False):
    model = LTXVideoTransformer3DModel(**config.diffusion_model.config)
    skip_load_from_pretrain = skip_load_from_pretrain or bool(
        getattr(config.diffusion_model, "skip_load_from_pretrain", False)
    )
    if load_weights and not skip_load_from_pretrain:
        if config.diffusion_model.model_path.endswith(".safetensors"):
            state_dict = load_file(config.diffusion_model.model_path)
        elif config.diffusion_model.model_path.endswith(".pt"):
            state_dict = torch.load(config.diffusion_model.model_path, map_location="cpu")
        else:
        ...
        print(">>> Loaded weights from pretrained checkpoint: %s"%config.diffusion_model.model_path)
    elif skip_load_from_pretrain:
        print(">>> Skipped pretrained LTX DiT checkpoint load")
    return model
