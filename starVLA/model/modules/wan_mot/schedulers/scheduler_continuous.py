...
class WanContinuousFlowMatchScheduler:
    """Continuous-time Flow-Matching scheduler."""
    ...
    def recover_sample_from_velocity(
        self,
        noisy_samples: torch.Tensor,
        pred_velocity: torch.Tensor,
        timestep: torch.Tensor,
    ) -> torch.Tensor:
        sigma = (timestep / float(self.num_train_timesteps)).to(noisy_samples.device, dtype=noisy_samples.dtype)
        if sigma.ndim == 0:
            return noisy_samples - sigma * pred_velocity
        sigma = sigma.view(-1, *([1] * (noisy_samples.ndim - 1)))
        return noisy_samples - sigma * pred_velocity
