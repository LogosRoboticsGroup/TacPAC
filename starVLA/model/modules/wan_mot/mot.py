...
from .wan_dit_adapters import flash_attention, modulate, rope_apply
...
class MoT(nn.Module):
    def __init__(
        ...
        self.time_alignment: Optional[nn.Module] = None
    ...
    def _build_expert_attention_io(
        ...
        freqs: torch.Tensor,
        t_mod: torch.Tensor,
        tau_ids: Optional[torch.Tensor] = None,
    ) -> tuple[
        torch.Tensor,
        torch.Tensor,
        torch.Tensor,
        ...
        if self.time_alignment is not None:
            q, k, v = self.time_alignment(q=q, k=k, v=v, tau_ids=tau_ids)
    ...
    def _apply_expert_post_block(
        ...
        cross_view_payload: Optional[dict] = None,
    ) -> torch.Tensor:
        x = block.gate(residual_x, gate_msa, block.self_attn.o(mixed_attn_out))

        if cross_view_payload is not None:
            x = cross_view_payload["expert"].apply_cross_view(
                layer_idx=cross_view_payload["layer_idx"],
                x=x,
                e_cv=cross_view_payload["e_cv"],
                n_view=int(cross_view_payload["n_view"]),
                tokens_per_view=int(cross_view_payload["tokens_per_view"]),
            )
    ...
    def _apply_post_with_optional_checkpoint(
        ...
        def _post_fn(
            ...
            _cross_view_payload=cross_view_payload,
        ) -> torch.Tensor:
            return self._apply_expert_post_block(
                ...
                cross_view_payload=_cross_view_payload,
            )
    ...
    def prefill_video_cache(
        ...
        video_tau_ids: Optional[torch.Tensor] = None,
    ...
    def forward_action_with_video_cache(
        ...
        Args:
            ...
            kv_out: Optional list; when provided, per-layer action `{"k","v"}` are appended
                (used by the tactile expert prefill).
        ...
        for layer_idx in range(self.num_layers):
            ...
            if kv_out is not None:
                kv_out.append({"k": k_action, "v": v_action})
    ...
    def forward(
        ...
        cross_view_all: Optional[Dict[str, Optional[dict]]] = None,
        tau_ids_all: Optional[Dict[str, torch.Tensor]] = None,
    ):
        ...
        for layer_idx in range(self.num_layers):
            ...
            # 1. 各个 expert 内部预处理，在 mixed self attention 前
            for name in self.expert_order:
                expert = self.mixtures[name]
                block = expert.blocks[layer_idx]
                x = tokens_all[name]
                ...
                tau_ids = None if tau_ids_all is None else tau_ids_all.get(name)
            ...
            # 2. concat all tokens for mixed attention
            ...
            for name, seq_len in zip(self.expert_order, seq_lens):
                ...
                cross_view_payload = None
                if cross_view_all is not None and cross_view_all.get(name) is not None:
                    cross_view_payload = dict(cross_view_all[name])
                    cross_view_payload.update({"expert": self.mixtures[name], "layer_idx": layer_idx})
                ...
                updated_tokens = self._apply_post_with_optional_checkpoint(
                    ...
                    cross_view_payload=cross_view_payload,
                )
