...
class Down_ResidualBlock(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, dropout, mult, temperal_downsample=False, down_flag=False):
        ...
        if down_flag:
            ...
            downsamples.append(Resample2_2(out_dim, mode=mode))
...
class Up_ResidualBlock(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, dropout, mult, temperal_upsample=False, up_flag=False):
        ...
        if up_flag:
            ...
            upsamples.append(Resample2_2(out_dim, mode=mode))
...
class Wan2_2_VAE_(WanVAE_):
    ...
    def __init__(self,
        ...
        self.encoder = Encoder3d2_2(dim, z_dim * 2, dim_mult, num_res_blocks,
                                    attn_scales, self.temperal_downsample, dropout)
        self.conv1 = CausalConv3d(z_dim * 2, z_dim * 2, 1)
        self.conv2 = CausalConv3d(z_dim, z_dim, 1)
        self.decoder = Decoder3d2_2(dec_dim, z_dim, dim_mult, num_res_blocks,
                                    attn_scales, self.temperal_upsample, dropout)
...
class WanVAE(nn.Module):
    ...
    def __init__(self, z_dim=16, vae_pretrained_path: str | None = None):
        ...
        self.mean = torch.tensor(LATENTS_MEAN_16CH, device='cuda')
        self.std = torch.tensor(LATENTS_STD_16CH, device='cuda')
        self.scale = [self.mean, 1.0 / self.std]

        self.model = WanVAE_(z_dim=z_dim).eval().requires_grad_(False)
...
class Wan2_2_VAE(WanVAE):

    def __init__(self, z_dim=48, dim=160, vae_pretrained_path: str | None = None):
        nn.Module.__init__(self)

        self.mean = torch.tensor(LATENTS_MEAN_48CH, device='cuda')
        self.std = torch.tensor(LATENTS_STD_48CH, device='cuda')
        self.scale = [self.mean, 1.0 / self.std]

        self.model = Wan2_2_VAE_(z_dim=z_dim, dim=dim).eval().requires_grad_(False)
