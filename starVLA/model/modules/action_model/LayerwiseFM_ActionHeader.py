...
class LayerwiseFlowmatchingActionHead(nn.Module):
    ...
    def _compute_action_loss(
        ...
        action_loss = F.mse_loss(pred_actions.float(), target_velocity.float(), reduction="none")
        ...
        if action_mask.ndim == 3:
            ...
            per_sample_loss = (action_loss * valid).sum(dim=(1, 2)) / valid.sum(dim=(1, 2)).clamp(min=1.0)
            return per_sample_loss.mean()
        if action_mask.ndim == 2:
            action_loss_token = action_loss.mean(dim=2)
            if action_mask.shape != action_loss_token.shape:
                raise ValueError(
                    "`action_mask` shape must match action timestep loss shape: "
                    f"mask={tuple(action_mask.shape)}, loss={tuple(action_loss_token.shape)}"
                )
            valid = action_mask.to(device=action_loss_token.device, dtype=action_loss_token.dtype)
            per_sample_loss = (action_loss_token * valid).sum(dim=1) / valid.sum(dim=1).clamp(min=1.0)
            return per_sample_loss.mean()
    ...
    def forward(
        ...
        action_mask: optional [B, action_horizon, D_action] or [B, action_horizon]
        """
        ...
        pred_actions = self.predict_velocity(
            vl_embs_list,
            noisy_trajectory,
            t_discretized,
            state=state,
        ...
        return self._compute_action_loss(pred_actions, velocity, action_mask)
