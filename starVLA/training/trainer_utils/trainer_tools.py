...
def resolve_gradient_accumulation_steps(cfg, num_processes: int) -> int:
    """Derive grad accumulation from trainer.global_batch_size (keeps the global
    batch constant across GPU counts), or fall back to trainer.gradient_accumulation_steps."""
    global_batch_size = getattr(cfg.trainer, "global_batch_size", None)
    if global_batch_size is None:
        return int(getattr(cfg.trainer, "gradient_accumulation_steps", 1))
    denom = int(cfg.datasets.vla_data.per_device_batch_size) * num_processes
    return max(1, int(global_batch_size) // denom)
...
def build_param_lr_groups(model, cfg):
    ...
    # assign base learning rate to the remaining unused parameters (exclude frozen or grad-less ones)
    other_params = [
...
class TrainerUtils:
    ...
    @staticmethod
    def _load_stage_checkpoint(model, checkpoint_path):
        """Load model weights from a previous stage without optimizer/scheduler state."""
        if not checkpoint_path:
            return model
        if dist.get_rank() == 0:
            print(f"📦 loading stage checkpoint: {checkpoint_path}")
        try:
            if _is_safetensors_path(checkpoint_path):
                from safetensors.torch import load_file
            ...
            raise RuntimeError(f"❌ loading stage checkpoint failed: {e}")
        ...
        if hasattr(model, "_before_load_state_dict"):
            checkpoint = model._before_load_state_dict(checkpoint)
        missing, unexpected = model.load_state_dict(checkpoint, strict=False)
        if hasattr(model, "_after_load_state_dict"):
            model._after_load_state_dict(checkpoint)
        if dist.get_rank() == 0:
            print(f"✅ loaded stage checkpoint with strict=False: missing={len(missing)}, unexpected={len(unexpected)}")
        return model
