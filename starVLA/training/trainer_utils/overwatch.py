...
# Set Logging Configuration — use programmatic setup so we can pass force_terminal=True
#   This ensures colored output is preserved even when stdout is piped (e.g. `| tee`)
...
_handler.setFormatter(logging.Formatter(RICH_FORMATTER, datefmt=DATEFMT))

logging.root.setLevel(logging.INFO)
logging.root.handlers = [_handler]
# Disable existing loggers to match previous dictConfig behavior
for name in logging.root.manager.loggerDict:
    logging.getLogger(name).disabled = True


def should_enable_progress_bar(local_main_process: bool | None = None, require_single_process: bool = False) -> bool:
    if local_main_process is None:
        local_main_process = _is_local_main_process()
    if not local_main_process:
        return False
    if require_single_process and _world_size() != 1:
        return False
    return True
