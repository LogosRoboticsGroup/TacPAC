...
## Server-side tactile preprocessing

All WebSocket and InferSystem inference requests share the tactile preprocessing
implemented by `baseframework.preprocess()`. Clients send raw tactile images;
the checkpoint's `datasets.vla_data` config selects `stress`, frame-0 residual,
or raw mode for every tactile view declared by its data mix. Policy reset clears
the online reference/baseline state. Server metadata publishes the tactile view
indices so the shared `M1Inference` client transports those views losslessly as PNG.
