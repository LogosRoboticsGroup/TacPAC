#!/bin/bash
# Multi-GPU WanMoTIDM training script (DeepSpeed ZeRO-2)
# Usage: bash train_WanMoTIDM.sh [data_mix] [run_id_suffix] [extra_args...]
# Example: bash train_WanMoTIDM.sh libero_all "debug" trainer.max_train_steps=1000
...
# $2 is treated as run_id_suffix only if it is not a CLI override.
if [ -n "$2" ] && [[ "$2" != *"="* ]] && [[ "$2" != --* ]]; then
  run_id_suffix=$2
...
accelerate launch \
  ...
  --framework.idm.video_cond_noise_prob 0.5 \
