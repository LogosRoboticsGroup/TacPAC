#!/bin/bash
# Multi-GPU LTXFM training script (DeepSpeed ZeRO-2).
# Usage: bash scripts/vla/train_LTXFM.sh [data_mix] [run_id_suffix] [extra_args...]
# Example: bash scripts/vla/train_LTXFM.sh libero_all_multi debug --trainer.max_train_steps 1000
...
###########################################################################################
# === Configuration ===
Framework_name=LTXFM
freeze_module_list=''
config_yaml=${CONFIG_YAML:-starVLA/config/training/vla/starvla_ltx_fm.yaml}
...
accelerate launch \
  ...
  --framework.video_model.num_frames 16 \
  --framework.action_model.action_horizon 64 \
